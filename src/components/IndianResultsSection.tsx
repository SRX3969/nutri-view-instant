...
import { Sparkles, RefreshCw, Save, AlertTriangle, Star, Apple, Flame, Droplets, Wheat, Cookie, Zap, Heart, Leaf, Scale } from "lucide-react";
...
export function IndianResultsSection({
  ...
  return <div className="w-full max-w-4xl mx-auto space-y-8 animate-fade-in">
      ...
      <Card className="overflow-hidden glass border-primary/20 animate-scale-in hover-lift relative">
        ...
        {data.nutritionScore && <div className={`absolute top-4 right-4 ${getScoreColor(data.nutritionScore)} text-white font-bold px-4 py-2 rounded-xl text-2xl shadow-lg`}>
            {data.nutritionScore}
          </div>}
      </Card>
      ...
      {/* Detected Items */}
      {data.detectedItems && data.detectedItems.length > 0 && <Card className="glass border-primary/20 p-6 space-y-4 animate-scale-in hover-lift">
          <h4 className="font-bold text-foreground text-xl flex items-center gap-2">
            <Apple className="h-5 w-5 text-primary" />
            Detected Items
          </h4>
          <div className="space-y-3">
            {data.detectedItems.map((item, index) => <div key={index} className="flex items-center justify-between bg-muted/50 rounded-xl p-4">
                <div>
                  <p className="font-medium text-foreground">{item.name}</p>
                  <p className="text-sm text-muted-foreground">{item.portion}</p>
                  {item.ingredients && <p className="text-xs text-muted-foreground mt-1">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        {data.fiber !== undefined && <Card className="glass border-primary/20 p-4 text-center animate-scale-in hover-lift">
      ...
      {/* Vitamins & Minerals */}
      {data.vitamins && Object.keys(data.vitamins).length > 0 && <Card className="glass border-primary/20 p-6 space-y-4 animate-scale-in hover-lift">
          <h4 className="font-bold text-foreground text-xl flex items-center gap-2">
            <Heart className="h-5 w-5 text-red-500" />
            Vitamins & Minerals (% Daily Value)
          </h4>
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            {data.vitamins.vitaminA !== undefined && <div className="space-y-2">
            ...
            {data.vitamins.vitaminC !== undefined && <div className="space-y-2">
                <div className="flex justify-between text-sm">
            ...
            {data.vitamins.iron !== undefined && <div className="space-y-2">
                <div className="flex justify-between text-sm">
            ...
            {data.vitamins.calcium !== undefined && <div className="space-y-2">
                <div className="flex justify-between text-sm">
      ...
      {/* Warnings */}
      {data.warnings && data.warnings.length > 0 && <Card className="glass border-red-500/20 p-6 space-y-4 animate-scale-in">
          <h4 className="font-bold text-foreground text-xl flex items-center gap-2">
          ...
          return <div key={index} className={`flex items-start gap-3 p-4 rounded-xl border ${getWarningColor(warning.type)}`}>
      ...
      {/* Recommendations */}
      {data.recommendations && data.recommendations.length > 0 && <Card className="glass border-primary/20 p-6 space-y-4 animate-scale-in hover-lift">
          <h4 className="font-bold text-foreground text-xl flex items-center gap-2">
            <Star className="h-5 w-5 text-primary" />
            Healthier Indian Alternatives
          </h4>
          <ul className="space-y-3">
            {data.recommendations.map((rec, index) => <li key={index} className="flex items-start gap-3 p-3 rounded-lg bg-primary/5 hover:bg-primary/10 transition-colors">
      ...
      {/* Tips */}
      {data.tips && data.tips.length > 0 && <Card className="glass border-primary/20 p-6 space-y-4 animate-scale-in hover-lift">
          <h4 className="font-bold text-foreground text-xl flex items-center gap-2">
            <Sparkles className="h-5 w-5 text-primary" />
            Nutrition Insights
          </h4>
          <ul className="space-y-3">
            {data.tips.map((tip, index) => <li key={index} className="flex items-start gap-3 p-3 rounded-lg hover:bg-primary/5 transition-colors">
      ...
      <div className="flex flex-col sm:flex-row gap-4 justify-center pt-4">
        {!isSaved && <Button onClick={handleSave} size="lg" className="gradient-nature hover:opacity-90 hover:scale-105 transition-smooth text-white font-bold px-10 py-7 text-lg rounded-xl shadow-lg">
            <Save className="mr-2 h-6 w-6" />
            Save to History
          </Button>}
        <Button onClick={onAnalyzeAnother} size="lg" className="gradient-gold hover:opacity-90 hover:scale-105 transition-smooth text-white font-bold px-10 py-7 text-lg rounded-xl shadow-lg pulse-glow">
